...
import {
  ...
  Sparkles, CheckCircle2, XCircle, Zap, BookOpen, ArrowRight, RotateCcw
} from 'lucide-react';
...
// ── Chat messages component (auto-scrolling, well-formatted) ─────────────────
interface ChatMsg { id: string; role: 'user' | 'assistant'; content: string; }

const ChatMessages = ({ messages, isTyping }: { messages: ChatMsg[]; isTyping: boolean }) => {
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={`max-w-[82%] rounded-xl px-3 py-2 text-xs leading-relaxed ${
              ...
              <div className="prose prose-xs prose-invert max-w-none [&>p]:mb-1 [&>p:last-child]:mb-0 [&>ul]:mb-1 [&>ul]:pl-3 [&>li]:mb-0.5 [&>h3]:text-xs [&>h3]:font-bold [&>h3]:mb-1 [&>strong]:text-foreground">
      ...
      {isTyping && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-2 justify-start">
          ...
          <div className="bg-muted border border-border rounded-xl rounded-bl-sm px-3 py-2.5 flex items-center gap-1">
            {[0, 150, 300].map(d => (
              <div key={d} className="w-1.5 h-1.5 rounded-full bg-accent/70 animate-bounce" style={{ animationDelay: `${d}ms` }} />
            ))}
          </div>
        </motion.div>
      )}
        ...
        <AnimatePresence>
          {selectedDomain && (
            ...
            >
              ...
              {/* Right: AI Chat — enhanced */}
              <CyberCard variant="accent" className="flex flex-col min-h-0" style={{ height: 'clamp(480px, 70vh, 680px)' }}>
                {/* Header */}
                <div className="flex items-center gap-3 pb-4 border-b border-border shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-display text-base font-bold leading-tight">AI Career Assistant</h3>
                    <p className="text-xs text-muted-foreground">Personalized resume coaching</p>
                  </div>
                  {resumeAnalysis && chatOpen && (
                    <button
                      onClick={() => { clearChat(); setChatOpen(false); }}
                ...
                {!chatOpen || !resumeAnalysis ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center px-4 py-6">
                    ...
                    <h4 className="font-display text-base font-bold mb-1.5">Career Chat</h4>
                    <p className="text-xs text-muted-foreground mb-5 max-w-[220px] leading-relaxed">
                      {resumeAnalysis
                        ? 'Resume analyzed! Start chatting for personalized insights.'
                        : 'Analyze your resume first to unlock the AI career assistant.'}
                    </p>
                    {resumeAnalysis && (
                      <CyberButton variant="accent" size="sm" onClick={() => setChatOpen(true)} glowing>
                  ...
                  <div className="flex flex-col flex-1 min-h-0 pt-3">
                    ...
                    {/* Quick prompts */}
                    <div className="shrink-0 px-0 py-2 flex flex-wrap gap-1.5">
                      {['Score breakdown', 'Missing skills', 'How to improve?', 'What to learn next?'].map(q => (
                        <button
                          key={q}
                          onClick={() => sendMessage(q)}
                          disabled={isTyping}
                          className="px-2.5 py-1 text-xs bg-muted/60 border border-border rounded-full hover:border-accent/60 hover:bg-accent/10 transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
                        >
                          {q}
                        </button>
                      ))}
                    ...
                    <div className="shrink-0 flex gap-2 pt-2 border-t border-border">
                      <Input
                        ...
                        className="flex-1 bg-muted/50 border-border text-sm h-9 focus:border-accent/50"
                      ...
                      <button
                        ...
                        className="w-9 h-9 shrink-0 flex items-center justify-center rounded-lg bg-accent/20 border border-accent/40 text-accent hover:bg-accent/30 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      >
